...
void printUtilitiesAndPolicy(vector<double>, vector<int>);

void initializeMDP(double, double, double, double);

void valueIteration(double, vector<double>&, vector<int>&, double);
...
int main (int argc, char* argv[]) {
	...
	double keyLossProbability = atof(argv[3]);
	double positiveTerminalReward = atof(argv[4]);
	double negativeTerminalReward = atof(argv[5]);
	double stepCost = atof(argv[6]);
	string iterArg = argv[7];

	//string iterArg(argv[7]);
	

	initializeMDP(stepCost, negativeTerminalReward, positiveTerminalReward, keyLossProbability);
	...
	for(int s = 0; s < NUM_STATES; s++) {
		...
		//Start with all utility of 0
		utility.push_back(0);

		//Start with all North
		policy.push_back(N);
	}

	valueIteration(epsilon, utility, policy, discount);
...
void valueIteration(double epsilon, vector<double> &utility, vector<int> &policy, double discount) {

	//delta is the maximum change in the utility of any state in an iteration
	double delta = 1;

	//Iterate until the maximum change in the utility of any state in an iteration is small enough
	 while(delta >= epsilon * (1.0 - discount) / discount){
	 	
	 	delta = 0;

		//Iterate over all states to find largest delta
		for(int s=0; s < NUM_STATES; s++) {
			
			double uPS = 0;

			//Action yielding the highest utility
			double maxAction = INT_MIN;
			...
			for(int a=0; a<NUM_ACTIONS; a++){
				...
				//Iterate over all s prime states to obtain the total utility for each action
				for(int sp=0; sp < NUM_STATES; sp++) {
					currSum += T[s][a][sp] * utility[sp];

				}

				if(currSum > maxAction){
					policy[s] = a;
					maxAction = currSum;
				}
			}

			uPS = R[s] + discount * maxAction;

			if(abs(uPS - utility[s]) > delta) {
				delta = abs(uPS - utility[s]);
			}
    ...
    //
void printUtilitiesAndPolicy(vector<double> utility, vector<int> policy) {
	...
	for(int s = 58 ; s <= 64 ; s += 2)
		cout << "(" << setw(2) << s << ") " << setw(5) << setprecision(PRINT_UTILITY_PRECISION) << utility[s] << "(" << action(policy[s]) << ")    ";
	cout << endl;
...
string action(int a) {
    ...
    // function
    //
void initializeMDP(double stepCost, double negativeTerminalReward, double positiveTerminalReward, double keyLossProbability) {
